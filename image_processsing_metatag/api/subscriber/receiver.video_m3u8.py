...
def main():
    try:
        ...
        subscribe_queue = 'video_m3u8_generate'
        ...
        def callback(ch, method, properties, body):
            try:
                ...
                if "message" in queue_data:
                    ...
                    servers_cred = queue_data["credentials"]
                    print(msg)
                    # check file exist
                    if os.path.exists(msg["src_video"]):
                        print(f"Valid Data Format {msg}")
                        # Start processing Data
                        stream_conversion = StreamConversionProcess
                        output = stream_conversion.GenerateM3u8AndProcessData(msg, servers_cred)
                        print(output)
                    else:
                        print("### NOT VALID FORMAT DATA ###")
                        errorData = dict()
                        errorData = queue_data
                        errorData["error_msg"] = "File Not Found"
                        errorData["queue_name"] = "not_found."+subscribe_queue

...
class CatalogueStarScore():
  ...
  # Return the highest score if value exceeds the last range
    ...
    def starDataNormalizedCatalogueScore(docid):
        DocId = [docid] if isinstance(docid, str) else docid
        ...
        with MongoClient(getConfigData('mongo.host'), username=username, password=password, authSource=auth_db) as client:
            ...
            if document_count > 0:
                print(f'docid {DocId} exists in collection')
                ...
                print(df.columns)  # Check the actual column names in the DataFrame
            else:
        ...
        # Debugging: Check the DataFrame after conversion
        print("DataFrame after fetching and filling NaN values:")
        print(df[['did', 'pid']]) 
    ...
    def dictConvert(quality_data,include_scr='0'):
        ...
        data_dict = quality_data.to_dict(orient='records')
        meta_data = {}
        ...
        for i in range(len(data_dict)):
            ...
            meta_data[i] = {
                ...
                'product_url': str(data_dict[i]['pul']),  # Corrected from 'pul' to 'product_url'
                'pid_score': data_dict[i]['star_score']
            }

            # Conditionally add detailed score data only if 'include_scr' is set to '1'
            if include_scr == '1':
                meta_data[i]['scr'] = {

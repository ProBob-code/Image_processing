...
class CatalogueScore():
    ...
    def dataNormalizedCatalogueScore(docid):
        ...
        with MongoClient(getConfigData('mongo.host'), username=username, password=password, authSource=auth_db) as client:
            ...
            query = {
                "$or": [
                    ...
                    {"did": {"$in": docids}, "df": 0, "dlf": 0, "ap": 1},
                    {"did": {"$in": docids}, "df": 0, "dlf": 0, "ap": 2},
                    {"did": {"$in": docids}, "df": 0, "dlf": 0, "ap": 0}
        ...
        try:
            ...
            # Calculate additional features and Avoid division by zero
            df['size_div_ppi'] = df['size'] / df['ppi'].replace(0, np.nan)
            df['size_div_ppi2'] = df['size'] ** 2 / df['ppi'].replace(0, np.nan)
            ...
            for i in range(len(D)):
                ...
                Norm_data = gf[['size_div_ppi2', 'ppi', 'blur_type', 'brightness_score', 'colourfulness', 'sharpness_score', 'artifact_verdict', 'partial_blur_status']].copy()
                ...
                if len(Norm_data) > 0:
                    ...
                    # Ensure 'quality_score' is numeric for later use
                    quality_data['quality_score'] = pd.to_numeric(quality_data['quality_score'], errors='coerce')
                    quality_data['quality_score'].fillna(0, inplace=True)
                ...
                final_data['classification'] = pd.to_numeric(final_data['classification'], errors='coerce').fillna(0)
                final_data['quality_score'] = pd.to_numeric(final_data['quality_score'], errors='coerce').fillna(0)
                final_data['likes_score'] = pd.to_numeric(final_data['likes_score'], errors='coerce').fillna(0)
                final_data['report_score'] = pd.to_numeric(final_data['report_score'], errors='coerce').fillna(0)
            ...
            print(f"Error in calculate_image_score: {e}")
            return final_data
...
class CategoryScore():
    ...
    def dictConvert(final_data):
        ...
        for i in range(len(data_dict)):
            ...
            meta_data[i] = {
                'national_catid': str(data_dict[i]['national_catid']),
...
class MetaCatalogueScore():
    ...
    def dataNormalizedMetaCatalogueScore(docid):
        start_time = time.time()
        DocId = docid
        docids = [docid.lower() for docid in DocId]

        # Connect to MongoDB server for df
        username = getConfigData("mongo.username")
        password = getConfigData("mongo.password")
        auth_db = getConfigData("mongo.auth_db")
        ...
        try:
            client = MongoClient(getConfigData('mongo.host'), username=username, password=password, authSource=auth_db)
            db = client[getConfigData('mongo.db')]
            collection = db[getConfigData('mongo.collection')]
            ...
            if document_count > 0:
                print(f'docid {DocId} exist in collection')
                cursor = collection.find(query)
                document_list = list(cursor)
                df = pd.DataFrame(document_list)
            else:
            ...
            print(f"An error occurred during df MongoDB query: {e}")
            return {"msg": f"An error occurred: {e}", "error_code": 1}
        finally:
            if client:
                client.close()
        ...
        # Connect to MongoDB server for df2
        client = None # Reset client
        try:
            client = MongoClient(getConfigData('mongo.host'), username=username, password=password, authSource=auth_db)
            db = client[getConfigData('mongo.db')]
            collection = db[getConfigData('mongo.collection_meta')]
            ...
            print(f"An error occurred during df2 MongoDB query: {e}")
            return {"msg": f"An error occurred: {e}", "error_code": 1}
        finally:
            if client:
                client.close()
        ...
        # --- Start of Modified Combine df and df2 section ---
        # Step 1: Start with df_final as a copy of df (from collection1)
        df_final = df.copy()

        # Step 2: Update the 'meta' column in df_final using values from df2
        # Create a temporary DataFrame from df2 with '_id' and 'meta'
        # This ensures only the 'meta' column from df2 is considered for replacement.
        df2_meta_update = df2[['_id', 'meta']].copy()

        # Merge df_final with this subset of df2 to bring in df2's 'meta' column.
        # We use a suffix to distinguish the 'meta' column coming from df2.
        df_final = pd.merge(df_final, df2_meta_update, on='_id', how='left', suffixes=('', '_from_df2'))

        # Prioritize 'meta_from_df2' where it exists (i.e., a match was found in df2).
        # If 'meta_from_df2' is NaN (no match in df2), it will keep the original 'meta' value from df.
        df_final['meta'] = df_final['meta_from_df2'].combine_first(df_final['meta'])

        # Drop the temporary column that was created during the merge
        df_final.drop(columns=['meta_from_df2'], errors='ignore', inplace=True)

        # Step 3: Add columns from df2 that are NOT present in the original df
        # Based on your provided column lists, 'purl' is the only column in df2 that is not in df.
        new_cols_from_df2_to_add = [col for col in df2.columns if col not in df.columns]

        # Ensure '_id' is not mistakenly added as a new column, as it's the merge key.
        if '_id' in new_cols_from_df2_to_add:
            new_cols_from_df2_to_add.remove('_id')

        if new_cols_from_df2_to_add:
            # Create a subset of df2 with '_id' and only these new columns
            df2_new_cols_subset = df2[['_id'] + new_cols_from_df2_to_add].copy()
            # Merge this subset into df_final to add these new columns.
            # 'how=left' ensures all rows from df_final are kept.
            df_final = pd.merge(df_final, df2_new_cols_subset, on='_id', how='left')

        # Assign the result back to df for subsequent operations in your function
        df = df_final
        # --- End of Modified Combine df and df2 section ---

        print(df_final.columns)
        print("df", df.head()) # Changed to df.head() for brevity

...
class ImageLSH:
    ...
    def data_conversion(docid, switch, content_status):
        ...
        for index, row in df.iterrows():
            ...
            filtered_ahash_similar_images = [{'pid': str(pid), 'h_score': round(float(similarity),3), 'h_hash': query_ahash, 'hash_name': 'ahash'} for pid, similarity in ahash_similarity_scores if 0.71 <= float(similarity) <= 1.00]
            filtered_dhash_similar_images = [{'pid': str(pid), 'h_score': round(float(similarity),3), 'h_hash': query_dhash, 'hash_name': 'dhash'} for pid, similarity in dhash_similarity_scores if 0.71 <= float(similarity) <= 1.00]
            filtered_phash_similar_images = [{'pid': str(pid), 'h_score': round(float(similarity),3), 'h_hash': query_phash, 'hash_name': 'phash'} for pid, similarity in phash_similarity_scores if 0.71 <= float(similarity) <= 1.00]
